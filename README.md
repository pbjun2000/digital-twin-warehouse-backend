...
제한된 프로젝트 기간 안에서
Warehouse State → AI Planning → Simulation으로 이어지는 핵심 실행 흐름을 우선 연결했습니다.
...
2개월 MVP에서는 사용자별 실행 환경을 명확하게 격리하고
구현·검증 범위를 단순하게 유지하는 것을 우선했습니다.

따라서 Shared Warehouse는 Template으로 두고
USER / GUEST별 Personal Warehouse를 Deep Clone하는 방식을 적용했습니다.

#### Trade-off

Deep Clone은 실행 상태를 명확하게 분리할 수 있다는 장점이 있지만,
Warehouse 데이터와 사용자 수가 증가할수록

- 데이터 복제 비용
- 저장 공간 사용량
- Personal Warehouse 생성 비용

이 증가할 수 있습니다.

서비스 규모가 커질 경우에는
공통 Warehouse Template은 공유하면서
**Runtime State만 사용자별로 분리하거나 Copy-on-write 방식으로 개선**하는 구조를 검토할 수 있습니다.
...
하지만 프로젝트에서는 Graph를 단순 계산에만 사용하는 것이 아니라
...
`AFTER_COMMIT` 방식은 PostgreSQL Transaction이 실패했는데
Neo4j만 먼저 갱신되는 문제는 방지할 수 있습니다.
...
와 같은 상황에서는 PostgreSQL과 Neo4j의 상태가 일시적으로 달라질 수 있습니다.

현재 MVP에서는 이 구조까지 구현했으며,
서비스 규모가 커질 경우 실패한 Graph Sync를 재처리할 수 있도록
**Retry 또는 Transactional Outbox 구조를 적용하는 방향**을 검토할 수 있습니다.
...
이를 통해 SimulationRun이 Personal Warehouse에 복제된 Scenario ID를 사용하도록 수정하고,
기존 Template Scenario ID가 실행 과정에서 잘못 사용되는 문제를 방지했습니다.
